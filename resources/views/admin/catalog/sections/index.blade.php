...
@section('title', __('view.admin.catalog.sections.index.title'))
    ...
    <div class="mb-auto container-fluid">
        ...
        <div class="card mb-3">
            <h2 class="card-header">
                {{ __('view.admin.catalog.sections.index.heading', ['count' => $count]) }}
            </h2>
        </div>
        <nav class="navbar navbar-light bg-light">
            <div class="container-fluid">
                <a href="{{ route('admin.sections.create') }}" type="button" class="btn btn-success">
                    <i class="bi bi-plus-circle"></i>
                    {{ __('view.admin.catalog.sections.index.add_section') }}
                </a>
                <form action="{{ route('admin.sections.index') }}" class="d-flex" method="GET">
                    <select class="form-select me-2" id="search_column" name="search_column">
                        <option value="id" @if (request()->query('search_column') == 'id') selected @endif>
                            {{ __('view.admin.catalog.sections.index.search_option_id') }}
                        </option>
                        <option value="name" @if (request()->query('search_column') == 'name') selected @endif>
                            {{ __('view.admin.catalog.sections.index.search_option_name') }}
                        </option>
                        <option value="created_at" @if (request()->query('search_column') == 'created_at') selected @endif>
                            {{ __('view.admin.catalog.sections.index.search_option_created_at') }}
                        </option>
                        <option value="updated_at" @if (request()->query('search_column') == 'updated_at') selected @endif>
                            {{ __('view.admin.catalog.sections.index.search_option_updated_at') }}
                        </option>
                    </select>
                    <input
                        id="search"
                        ...
                        type="search"
                        placeholder="{{ __('view.admin.catalog.sections.index.search_placeholder') }}"
                        aria-label="Search"
                    >
                    <button class="btn btn-secondary" type="submit">
                        {{ __('view.admin.catalog.sections.index.search_button') }}
                    </button>
                </form>
            </div>
        </nav>
                            ...
                            <tr>
                                <th scope="col">
                                    <button
                                        ...
                                        value="id"
                                    >
                                        {{ __('view.admin.catalog.sections.index.sort_id') }}
                                    </button>
                                </th>
                                <th scope="col">
                                    <button
                                        ...
                                        value="name"
                                    >
                                        {{ __('view.admin.catalog.sections.index.sort_name') }}
                                    </button>
                                </th>
                                <th scope="col">
                                    <button
                                        ...
                                        value="created_at"
                                    >
                                        {{ __('view.admin.catalog.sections.index.sort_created_at') }}
                                    </button>
                                </th>
                                <th scope="col">
                                    <button
                                        ...
                                        value="updated_at"
                                    >
                                        {{ __('view.admin.catalog.sections.index.sort_updated_at') }}
                                    </button>
                                </th>
                            </tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center align-items-center">
                                        ...
                                        <form action="{{ route('admin.sections.destroy', $section->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger deleteSectionButton" data-confirm-message="{{ __('view.admin.catalog.sections.delete_confirm') }}"><i
